...
public:
  ...
  std::shared_ptr<PGUPV::Mesh> crearCilindro(float originY, float r_bottom, float r_top, float height, uint stacks,
      uint slices, const glm::vec4& color, bool mitad);
...
private:
  ...
  std::shared_ptr<Texture2D> texSuelo;
  std::shared_ptr<Texture2D> texPared;
  GLint texUnitLoc;
  Mirror espejo;
...
std::shared_ptr<PGUPV::Mesh> MyRender::crearCilindro(float originY, float r_bottom, float r_top, float height, uint stacks,
    uint slices, const glm::vec4& color, bool mitad=false) {
    ...
    for (uint j = 0; j <= stacks; j++) {
        ...
        for (uint i = 0; i <= slices; i++, angle += delta) {
            ...
            tex_coord.push_back(glm::vec2(angle / TWOPIf * 10, (float)j / stacks));
        }
    }
...
void MyRender::crearCaja(float size, float height, float width, float originY, const glm::vec4& color) {
    ...
    /*vertices = {
        ...
        glm::vec3(-s2, height / 8 + originY, s2), glm::vec3(s2, height / 8 + originY, s2),
        glm::vec3(-s2, height / 8 + originY, -s2), glm::vec3(s2, height / 8 + originY, -s2)
    };
    crearTriangleStrip(vertices, color);*/
...
void MyRender::setup() {
  ...
  // Creamos el modelo de la fuente
  ...
  // cilindro
  fuente.addMesh(crearCilindro(0.75f, 0.1f, 0.1f, 1.5f, 20U, 20U, colorCilindro));
  // cono
  fuente.addMesh(crearCilindro(1.55f, 0.1f, 0.0001f, 0.1f, 20U, 20U, colorCilindro));

  // Creamos el modelo del suelo y la pared
  glm::vec4 colorSuelo = glm::vec4(0.5f, 0.5f, 0.3f, 1.0f);

  // suelo
  texSuelo = std::make_shared<Texture2D>();
  texSuelo->loadImage("../recursos/imagenes/suelo.jpg");
  suelo.addMesh(crearCilindro(-0.001f, 0.0f, 4.0f, 0.0f, 30U, 30U, colorSuelo));

  // pared
  texPared = std::make_shared<Texture2D>();
  texPared->loadImage("../recursos/imagenes/pared.jpg");
  pared.addMesh(crearCilindro(1.0f, 4.0f, 4.0f, 2.0f, 20U, 20U, colorSuelo, true));

  //glBlendColor(0.0f, 0.0f, 0.0f, 0.8f);
...
void MyRender::render() {
  ...
  // Creación suelo y pared
  glStencilMask(0x00);
  glStencilFunc(GL_NOTEQUAL, 2, 0xFF);
  glCullFace(GL_FRONT);
  mats->pushMatrix(GLMatrices::MODEL_MATRIX);
  mats->setMatrix(GLMatrices::MODEL_MATRIX, glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f)));
  ashader.use();
  glEnable(GL_CULL_FACE);
  glCullFace(GL_FRONT);
  texSuelo->bind(GL_TEXTURE0);
  suelo.render();
  texPared->bind(GL_TEXTURE0);
  pared.render();
  glDisable(GL_CULL_FACE);
  litProgram.use();
  mats->popMatrix(GLMatrices::MODEL_MATRIX);

  // Creación suelo y pared reflejado
  glStencilMask(0x00);
